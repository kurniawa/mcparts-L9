...
<div class="threeDotMenu">
    ...
    <div class="divThreeDotMenuContent">
        ...
        <form action="/srjalan/srjalan-printOut" method='POST'>
            ...
            <input type="hidden" name="srjalan" value='{{ json_encode($srjalan) }}'>
            <input type="hidden" name="pelanggan" value='{{ json_encode($pelanggan) }}'>
            <input type="hidden" name="reseller" value='{{ json_encode($reseller) }}'>
            <input type="hidden" name="ekspedisi" value='{{ json_encode($ekspedisi) }}'>
            @csrf
        </form>
...
<script>
    ...
    `;
    for (var i = 0; i < d_srjalan_item.length; i++) {
        ...
        <div id="divOpsiEdit-${i}" style="display:none">
            ...
            <div id="divInputJumlah-${i}" class="mt-0_5em" style="display:none">
                Jumlah tersedia:
                Jumlah Tambahan yang Ingin diinput: <input class="p-0_5em" type="number" value="${d_srjalan_item[i].jml_item}">
            </div>
            ...
            <div id="divInputNamaHrg-${i}" style="display:none">
                <div class="mt-0_5em">Nama srjalan: <input class="p-0_5em w-70" type="text" value="${d_srjalan_item[i].nama_srjalan}"></div>
                <div class="mt-0_5em">Hrg/pcs: <input class="p-0_5em" type="number" value="${d_srjalan_item[i].hrg_item}"></div>
            </div>
            <br>
        ...
        // console.log(`d_srjalan_item[${i}].jml_item: ${d_srjalan_item[i].jml_item}`);
        // console.log(`d_srjalan_item[${i}].nama_srjalan: ${d_srjalan_item[i].nama_srjalan}`);
        // console.log(`d_srjalan_item[${i}].hrg_item: ${d_srjalan_item[i].hrg_item}`);
        // console.log(`d_srjalan_item[${i}].hrg_t: ${d_srjalan_item[i].hrg_t}`);
        ...
        htmlItem += `
            <div class="bb-1px-solid-grey pb-0_5em pt-0_5em">
                <div>${d_srjalan_item[i].jml_item}</div>
                <div>${d_srjalan_item[i].nama_nota}</div>
                <div>${d_srjalan_item[i].colly}</div>
            </div>
        `;
        // console.log(htmlItem);
        // totalHarga += parseInt(d_srjalan_item[i].hrg_t);
    }
    htmlItem += '</div>';
    ...
    var htmlTotalHarga =
        ...
        <div class="text-right p-1em">
            <div class="font-weight-bold font-size-2em color-green">${srjalan.colly_T}</div>
