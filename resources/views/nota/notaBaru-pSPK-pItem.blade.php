...
<script>
    ...
    for (let i_spks = 0; i_spks < spks.length; i_spks++) {
        ...
        for (let i_arrAvNotaItems = 0; i_arrAvNotaItems < arr_av_nota_items[i_spks].length; i_arrAvNotaItems++) {
            ...
            if (arr_av_nota_items[i_spks][i_arrAvNotaItems].jml_selesai === 0) {
                input_disabled = true;
            }
            /*
            ...
            */
            ...
            var jml_sdh_nota = 0;
            if (arr_av_nota_items[i_spks][i_arrAvNotaItems].data_nota !== null && arr_av_nota_items[i_spks][i_arrAvNotaItems].data_nota !== '') {
                var data_nota = JSON.parse(arr_av_nota_items[i_spks][i_arrAvNotaItems].data_nota);
                console.log('data_nota');
                console.log(data_nota);
                for (let index = 0; index < data_nota.length; index++) {
                    jml_sdh_nota += parseInt(data_nota[index].jml_item);
                }
            }
            // if (condition) {
        ...
        // console.log('input_disabled', input_disabled);
